(function ()
{
	...
	var url_image = './img/minesweeper_graphs.png', content = document.getElementById('mineSweeper');
	...
	function selectedElement (posX, posY)
	{
		...
		{
			if (!(posY >= y * tileSize && posY <= y * tileSize + tileSize))
				continue;
	...
	function onDoubleClick (event)
	{
		...
		if (!_el.covered && !_el.flag && !_el.suspect && noOneIsDetonated() && !clearIsUncovered())
		{
			...
			{
				if (!tabuleiro[_el.y + y])
					continue;
				for (var x = -1; x <= 1; x++)
				{
					var celula_vizinha = tabuleiro[_el.y + y][_el.x + x];
					if (!celula_vizinha || !y && !x)
						continue;
					if (celula_vizinha.flag === 'bombFlag')
						howMany++;
				}
			}
	...
	function clearAround (el, matriz)
	{
		for (var i = -1; i <= 1; i++)
		{
			...
			for (var j = -1; j <= 1; j++)
			{
				...
				if (!vizinha || vizinha.flag === 'bombFlag')
					continue;
